...
class actionSendSms extends actionBase
{
    ...
    function run_action(SugarBean $bean,$params = array())
    {
		...
		global $db;
		$GLOBALS['log']->fatal("In Run scirpt");
        try{
			...
			$GLOBALS['log']->fatal($lbn_lbn->to_sms);
    ...
    function edit_display($line, SugarBean $bean = null, $params = array())
    {
		global $app_list_strings;
		global $db;
		$query = $db->pquery("select id,name from smsad_sms_template where deleted='0'");
		while($value = $db->fetchByAssoc($query))	{
			$sms_template_array[$value['id']] = $value['name'];
		}
		$app_list_strings['aow_sms_type_list'] = array('phone_mobile' => 'Mobile Phone', 'phone_office' => 'Office Phone');
